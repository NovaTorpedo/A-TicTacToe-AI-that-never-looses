...
def minimax(board):
    ...
    if terminal(board):
        return None
    #case player is x(max_player)
    elif player(board) == X:
        plays = []
        #loop over all the possible actions
        for action in actions(board):
            #add a list of all the possible plays to achieve min_value and the action that results to its value
            plays.append([min_value(result(board,action)),action])
        #reverse sort for the plays list and get the action that should take
        return sorted(plays, key=lambda x: x[0], reverse=True)[0][1]
    
    #case player is O(min_player)
    elif player(board) == O:
        plays = []
        #loop over all the possible actions
        for action in actions(board):
            #add a list of all the possible plays to achieve max_value and the action that results to its value
